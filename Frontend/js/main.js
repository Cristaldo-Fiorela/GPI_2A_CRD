...
/* ======= Mapeos API ↔ UI =======
 * API  -> UI fila: { id, foto_url, nombre, apellido, Puesto, descripcion }
 * UI   -> HTML: Foto | Nombre | Rol | Correo(texto)
 */
...
document.getElementById("abrir-modal-nuevo")
  .addEventListener("click", () => abrir("modal-nuevo"));

formNuevo.addEventListener("submit", async (e) => {
  e.preventDefault();

  const uiData = {
    nombreCompleto: (nuevoNombre.value || "").trim(),
    correoTexto: (nuevoCorreo.value || "").trim(), // en tu API va a 'descripcion'
    rol: nuevoRol.value,
    fotoUrl: "" // si más adelante agregás un input de foto, mapealo acá
  };
  if (!uiData.nombreCompleto || !uiData.rol) return;

  try {
    await integrantesService.create(uiData);
